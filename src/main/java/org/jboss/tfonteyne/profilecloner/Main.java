...
public class Main {
    ...
    private static void usage() {
        System.out.println(
                "JBoss AS 7 / WildFly / JBoss EAP 6  Profile (and more) Cloner - by Tom Fonteyne - version:"
                        + VERSION);
    ...
    private boolean readOptions(String[] args) {
        ...
        while ((i < args.length) && (args[i] != null)
            ...
            if (args[i].startsWith("--controller=")) {
                ...
                addDeployments = Boolean.parseBoolean(
                        args[i++].substring("--add-deployments=".length()));
            } else if ("-ad".equals(args[i])) {
                addDeployments = true;
                i++;
            } else if (args[i].startsWith("--port=")) {
                port = Integer
                        .parseInt(args[i++].substring("--port=".length()));
            } else {
                return false;
            }
        }
